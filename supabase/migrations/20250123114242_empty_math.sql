...
SELECT 
  ...
  COALESCE(ts.ticket_details, '[]'::json) as ticket_details
FROM orders o
LEFT JOIN events e ON e.id = o.event_id
LEFT JOIN profiles p ON p.user_id = o.user_id
LEFT JOIN ticket_summary ts ON ts.order_id = o.id;

-- Create improved view for ticket details
...
BEGIN
  ...
  FOR ticket_type IN 
    SELECT 
      tt.price,
      (jsonb_array_elements(NEW.ticket_quantities)->>'quantity')::integer as quantity
